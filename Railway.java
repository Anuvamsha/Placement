...
public class Main {
    ...
    static String place[] = { "Place -> 1", "Place -> 2", "Place -> 3", "Place -> 4", "Place -> 5", "Place -> 6", "Place -> 7" };
        ...
        while (true) {
            if (Admin_Attempt >= 0) {
                ...
                if (id.equals(Admin_Id) && pass.equals(Admin_Password)) {
                    ...
                    Clr_scr();
                    Admin_Attempt -= 1;
                    System.out.println("Id and Password mismatch Please try again ! You have only " + Admin_Attempt + " Attempt's");
                }
            }
            else {
                Clr_scr();
        ...
        ab: while (true) {
            ...
            while (true) {
                ...
                if (start > 0 && start < 8) {
                    ...
                    System.out.println("Invalid Starting Pointt...");
                }
            }
            int landing;
            while (true) {
            ...
            seatid++;
            if (seat1 != -1 && seat2 == 0) {
                Booking.add(new Booking_S(User_List.get(Current_User).U_Id, (seat1 + 1), bookingid, place[start - 1], place[landing - 1], start, landing, "Seat Alloted"));
                System.out.println("Seat Allotted :" + (seat1 + 1));
                System.out.println("Booking id :" + bookingid);
                bookingid++;
            }
            else if (seat1 != -1 && seat2 == 1) {
                Waiting.add(new Waiting_S(User_List.get(Current_User).U_Id, (seat1 + 1), bookingid, place[start - 1], place[landing - 1], start, landing, "Seat Waitinglist"));
                System.out.println("WaitSeat Allotted :" + (seat1 + 1));
                System.out.println("Booking id :" + bookingid);
                bookingid++;
            }
            else {
    ...
    static void History() {
        ...
        for (int i = 0; i <Booking.size(); i++) {
            if (Booking.get(i).User_Id == User_List.get(Current_User).U_Id) {
                ...
                System.out.println("Landing Point : " + Booking.get(i).Landing_Point);
                System.out.println("Seat Number : " + Booking.get(i).Alloted_Seat);
        ...
        for (int i = 0; i <Waiting.size(); i++) {
            if (Waiting.get(i).User_Id == User_List.get(Current_User).U_Id) {
                ...
                System.out.println("Landing Point : " + Waiting.get(i).Landing_Point);
                System.out.println("Seat Number : " + Waiting.get(i).Alloted_Seat);
...
class Booking_S {
    ...
    public Booking_S(int User_Id, int Alloted_Seat, int Booked_Id, String StartPt, String EndPt, int St_Id, int Ld_Id, String Status) {
...
class Waiting_S {
    ...
    public int Landing_Id;
    public String Status;
    public Waiting_S(int User_Id, int Alloted_Seat, int Booked_Id, String StartPt, String EndPt, int St_Id, int Ld_Id, String Status) {
        this.User_Id = User_Id;
        this.Alloted_Seat = Alloted_Seat;
        this.Booking_Id_List = Booked_Id;
        this.Starting_Point = StartPt;
